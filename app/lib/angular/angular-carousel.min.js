...
angular.module("angular-carousel", ["ngMobile"]), angular.module("angular-carousel").directive("rnCarouselIndicators", [function () {
    return{restrict: "A", replace: !0, scope: {items: "=", index: "="}, template: '<div class="rn-carousel-indicator"><span ng-repeat="item in items" ng-class="{active: $index==$parent.index}">●</span></div>'}
    ...
    return{restrict: "EA", transclude: !0, replace: !0, scope: !0, template: "<ul rn-carousel rn-carousel-buffered><li ng-transclude></li></ul>", compile: function (e, n) {
        ...
        return e.find("li").attr("ng-repeat", i), function (e, n, i) {
...
}]), angular.module("angular-carousel").directive("rnCarousel", ["$compile", "$parse", "$swipe", "$document", "$window", "CollectionManager", function (t, e, n, i, s, r) {
    var o = 0;
    return{restrict: "A", scope: !0, compile: function (a, l) {
        a.addClass("rn-carousel-slides");
        var u = a.find("li")[0].attributes, c = u["ng-repeat"];
        if (c || (c = u["data-ng-repeat"]), c || (c = u["x-ng-repeat"]), !c)throw Error("carousel: cannot find the ngRepeat attribute");
        var f = c.value.match(/^\s*(.+)\s+in\s+(.*?)\s*(\s+track\s+by\s+(.+)\s*)?$/), h = f[1], d = f[2], p = f[3] || "", g = angular.isDefined(l.rnCarouselBuffered);
        return c.value = h + " in carouselCollection.cards " + p, function (a, l, u) {
            function c(t) {
                var e = angular.element(t).css("-webkit-transform").match(/translate3d\((-?\d+(?:px)?), (-?\d+(?:px)?), (-?\d+(?:px)?)\)/);
                return e ? e.slice(1, 3) : [0, 0, 0]
            }

            function f(t) {
                !t.target || t.target !== D[0] || "transform" !== t.propertyName && "-webkit-transform" !== t.propertyName && "-moz-transform" !== t.propertyName || (a.$apply(function () {
                    m(), a.carouselCollection.adjustBuffer(), y(!0)
                }), D.css("transform", "").css(C(c(D[0]), !1)))
            }

            function h(t, e) {
                function n() {
                    T = !0, a.carouselCollection[t](e, !0)
                }
            ...
            function m() {
                ...
                0 === t && angular.isDefined(u.rnCarouselPrev) && (i = e(u.rnCarouselPrev)(a, {item: a.carouselCollection.cards[0]}), p("before", i)), t === n && angular.isDefined(u.rnCarouselNext) && (i = e(u.rnCarouselNext)(a, {item: a.carouselCollection.cards[a.carouselCollection.cards.length - 1]}), p("after", i))
            }

            function x(t, e) {
                var n = {};
                return n[t] = e, angular.forEach(P, function (i) {
                    n["-" + i.toLowerCase() + "-" + t] = e
                }), n
            }

            function C(t, e) {
                return e ? x("transform", "translate3d(" + t + "px,0,0)") : x("transform", "translate(" + t + "px,0)")
            }

            function v() {
                E.css("width", "auto"), T = !0;
                var t = D.find("li");
                return z = 0 === t.length ? D[0].getBoundingClientRect().width : t[0].getBoundingClientRect().width, E.css("width", z + "px"), z
            }

            function y(t) {
                T = !!t || T, 0 === z && v(), B = a.carouselCollection.getRelativeIndex() * -z, T === !0 ? D.removeClass("rn-carousel-animate").addClass("rn-carousel-noanimate").css(C(B, !1)) : D.removeClass("rn-carousel-noanimate").addClass("rn-carousel-animate").css(C(B, !0)), T = !1
            }

            function b(t) {
                if (i.unbind("mouseup", I), 0 === z && v(), $ > 1) {
                    var e = a.carouselCollection.getLastIndex(), n = a.carouselCollection.position, s = S > B ? 1 : -1, r = Math.min(Math.max(0, n + s), e), o = t.x - M;
                    z * L >= Math.abs(o) && (r = n);
                    var l = n !== r;
                    l ? a.$apply(function () {
                        angular.isDefined(u.rnCarouselCycle) && (a.carouselCollection.position = r, y()), a.carouselCollection.goTo(r, !0)
                    }) : a.$apply(function () {
                        y()
                    })
                }
            ...
            o++;
            var w = "rn-carousel-" + o, $ = 0, M = 0, S = 0, B = 0, L = .1, z = 0, T = !0, D = l.wrap("<div id='" + w + "' class='rn-carousel-container'></div>"), E = D.parent(), A = e(d), j = {}, k = 0;
            if (u.rnCarouselIndex) {
                var N = e(u.rnCarouselIndex);
                angular.isFunction(N.assign) ? (a.$watch("carouselCollection.index", function (t) {
                    N.assign(a.$parent, t)
                }), k = N(a), a.$parent.$watch(N, function (t) {
                    void 0 !== t && a.carouselCollection.goToIndex(t, !0)
                })) : isNaN(u.rnCarouselIndex) || (k = parseInt(u.rnCarouselIndex, 10))
            }
            angular.isDefined(u.rnCarouselCycle) && (j.cycle = !0), j.index = k, g && (j.bufferSize = 3, j.buffered = !0), a.carouselCollection = r.create(j), a.$watch("carouselCollection.updated", function (t) {
                t && y()
            });
            var O = !1;
            a.$watch(A, function (t) {
                a.carouselCollection.setItems(t, O), O = !0, 0 === z && v(), y()
            }), angular.isDefined(u.rnCarouselWatch) && a.$watch(d, function (t) {
                a.carouselCollection.setItems(t, !1), O = !0, 0 === z && v(), y()
            }, !0);
            var P = ["webkit", "moz"];
            if (D[0].addEventListener("webkitTransitionEnd", f, !1), D[0].addEventListener("transitionend", f, !1), window.addEventListener("orientationchange", function () {
                v(), y()
            }), angular.isDefined(u.rnCarouselIndicator)) {
                var R = t("<div id='" + w + "-indicator' index='carouselCollection.index' items='carouselCollection.items' data-rn-carousel-indicators class='rn-carousel-indicator'></div>")(a);
                E.append(R)
            }
            var F = null, W = "iPad" == s.navigator.platform ? 0 : 50;
            n.bind(D, {start: function (t) {
                0 === $ && ($ = 1, M = t.x), i.bind("mouseup", I)
            }, move: function (t) {
                if (0 !== $) {
                    var e = t.x - M;
                    if (1 === $ && 0 !== e)$ = 2, S = B; else if (2 === $) {
                        var n = (new Date).getTime();
                        if (F && W > n - F)return;
                        F = n;
                        var i = a.carouselCollection.getLastIndex(), s = a.carouselCollection.position, r = 1;
                        (0 === s && t.x > M || s === i && M > t.x) && (r = 3), B = S + e / r, D.css(C(B, !0)).removeClass("rn-carousel-animate").addClass("rn-carousel-noanimate")
                    }
                }
            }, end: function (t) {
                b(t)
